...
char commandbuf[30];
bool FanSpeedMenu; // true back to temp menu // false back to adjust menu
unsigned int FanOn = 255;
...
#if ENABLED(FILAMENT_RUNOUT_SENSOR)//ENABLED(MachineCR10SPro) || ENABLED(AddonFilSensor)
...
void RTSSHOW::RTS_Init()
{
	...
	#if HAS_MESH //&& (ENABLED(MachineCR10SPro) || ENABLED(Force10SProDisplay)) // FIX ME
  ...
  RTS_SndData(ExchangePageBase, ExchangepageAddr); // RESET Page back to "0" -> Boot screen
  RTS_SndData(0,StartIcon); // RESET Loading bar to zero
  RTS_SndData(0,StartIcon+1); // RESET Loading bar to zero
  ...
  RTS_SndData(0, SoftVersion);
  RTS_SndData(SHORT_BUILD_VERSION, SoftVersion);
  RTS_SndData(0, PrinterSize);
  RTS_SndData(sizebuf, PrinterSize);
  RTS_SndData(0, CorpWebsite);
...
// FIX ME - MAKE ME BETTER WAY !
void RTSSHOW::RTS_SndDataL(unsigned long iVal, int iAdr)//Send iVal for VP= iAdr to DGUS
{
  byte bAdrL, bAdrH;
  bAdrL=iAdr&0xFF;
  bAdrH=(iAdr>>8)&0xFF;
...
void RTSSHOW::RTS_HandleData()
{
    ...
    for(int i = 0;Addrbuf[i] != 0;i++) // CASE
    {
  	  if(recdat.addr == Addrbuf[i] || /*AdvancedMenu*/ Addrbuf[i] >= AdvancedMenu && Addrbuf[i] <= AdvancedMenu+110)
  	  {
  	  	if(Addrbuf[i] >= Stopprint && Addrbuf[i] <= Resumeprint)
  			...
  			Checkkey = Filement;
    		  else if(Addrbuf[i] == SpeedFan) // SpeedFan
    	  Checkkey = SpeedFan;
          else if(Addrbuf[i] == Volume) // Volume
        Checkkey = Volume;
          else if(Addrbuf[i] == SettingPage) // Setting
        Checkkey = SettingPage;
          else if(Addrbuf[i] == ReturnBack) // ReturnBack
        Checkkey = ReturnBack;
          else if(Addrbuf[i] == Printfile) // Printfile
        Checkkey = Printfile;
          else if(Addrbuf[i] == Ajust) // Adjust
        Checkkey = Ajust;
          else if(Addrbuf[i] == Feedrate) // Feedrate
        Checkkey = Feedrate;
          else if(Addrbuf[i] == FlowRate) // FlowRate
        Checkkey = FlowRate;
          else if(Addrbuf[i] == Zoffset) // Zoffset
        Checkkey = Zoffset;
          else if(Addrbuf[i] == TempControl) // TempControl
        Checkkey = TempControl;
          else if(Addrbuf[i] == ManualSetTemp) //  ManualSetTemp
        Checkkey = ManualSetTemp;
          else if(Addrbuf[i] == Bedlevel) // Bedlevel
        Checkkey = Bedlevel;
          else if(Addrbuf[i] >= AdvancedMenu && Addrbuf[i] <= AdvancedMenu+100) // FIX ME
    		Checkkey = AdvancedMenu;
          else
    		Checkkey = i;
    		break;
    	  }
		...
		else if(recdat.data[0] == 3 )	// Temperature control
		{
		  ...
		  TPShowStatus = false;
			  SERIAL_ECHO("\n Handle Data PrintFile 3 Setting Screen ");
				RTS_SndData(ExchangePageBase + 57, ExchangepageAddr); //exchange to 57 page
		}
  ...
  case FlowRate : // DEF OK
	...
	case PrintChoice: // DEF FIX ME ?
		...
		else if(recdat.addr == Resumeprint && recdat.data[0] == 1)
		{
			...
			RTS_SndData(2+CEIconGrap,IconPrintstatus);
			PrintStatue[1] = 0;
			Update_Time_Value = 0;
	...
	case TempControl: // DEF OK
		...
		else if(recdat.data[0] == 3)
		{
			if(FanStatus)	//turn on the fan
			{
				fanSpeeds[0] = FanOn;
				FanStatus = false;
				RTS_SndData(ExchangePageBase + 38, ExchangepageAddr); //exchange to 38 page, the fans on
			}
			else//turn off the fan
			{
				fanSpeeds[0] = FanOff;
				FanStatus = true;
				RTS_SndData(ExchangePageBase + 39, ExchangepageAddr); //exchange to 39 page, the fans on
			}
		}
	...
	case ManualSetTemp: // DEF OK
		if(recdat.addr == NzBdSet) // % ?
		{
			...
			{
        enqueue_and_echo_commands_P((PSTR("M104 S0 T0")));
				RTS_SndData(0,NozzlePreheat);
			}
			else if(recdat.data[0] == 2)
			{
	...
	case Bedlevel: // DEF OK
			...
			else if(recdat.data[0] == 6) 	// Assitant Level ,  Centre 1 // OK
			{
				waitway = 4; //only for prohibiting to receive massage
			...
			else if(recdat.data[0] == 7) 	// Assitant Level , Front Left 2 OK
			{
				waitway = 4;		//only for prohibiting to receive massage
				enqueue_and_echo_commands_P((PSTR("G1 F100 Z3;")));
                char comdbuf[30];
			...
			else if(recdat.data[0] == 8) 	// Assitant Level , Front Right 3 // OK
			{
				waitway = 4;		//only for prohibiting to receive massage
				enqueue_and_echo_commands_P((PSTR("G1 F100 Z3;")));
				char comdbuf[30];
			...
			else if(recdat.data[0] == 9) 	// Assitant Level , Back Right 4 // OK
			{
				waitway = 4;		//only for prohibiting to receive massage
				enqueue_and_echo_commands_P((PSTR("G1 F100 Z3;")));
				char comdbuf[30];
			...
			else if(recdat.data[0] == 10) 	// Assitant Level , Back Left 5 OK
			{
				waitway = 4;		//only for prohibiting to receive massage
				enqueue_and_echo_commands_P((PSTR("G1 F100 Z3;")));
				char comdbuf[30];
    ...
    if(recdat.data[0] == 2)
		 {
       if(FanSpeedMenu)
        RTS_SndData(ExchangePageBase + 57, ExchangepageAddr); //exchange to 57 page, Adjust menu
       else
        RTS_SndData(ExchangePageBase + 56, ExchangepageAddr); //exchange to 56 page, Temperature menu
     }
     else if(recdat.data[0] == 4) // FAN - MAKE BETTER
     {
  		if(FanStatus)	//turn on the fan
  		{
        RTS_SndData(FanOn, SpeedFan);
        ...
        RTS_SndData(ExchangePageBase + 38, ExchangepageAddr); //exchange to 38 page, the fans on
  		...
  		else//turn off the fan
  		{
  			...
  			fanSpeeds[0] = FanOff;
  			FanStatus = true;
        RTS_SndData(ExchangePageBase + 39, ExchangepageAddr); //exchange to 39 page, the fans off
  		}
		}
    else if(recdat.data[0] == 5) // *MENU..
    {
      FanSpeedMenu = true;
    if(FanStatus)
      RTS_SndData(ExchangePageBase + 39, ExchangepageAddr); //exchange to 39 page, the fans off
    ...
    else if(recdat.data[0] == 6) // *ADJUST
    {
      FanSpeedMenu = false;
    if(FanStatus)
      RTS_SndData(ExchangePageBase + 39, ExchangepageAddr); //exchange to 39 page, the fans off
    ...
    else if(recdat.data[0] >= 50 and recdat.data[0] < 256) // FAN - MAKE BETTER
    {
      FanOn = recdat.data[0];
      fanSpeeds[0] = recdat.data[0];
    }
    break;

    case AdvancedMenu : // Advanced
    if(recdat.addr == AdvancedMenu+80) // AdvancedMenu
		 {
       RTS_SndDataL(planner.axis_steps_per_mm[A_AXIS]*100, AdvancedMenu);   // X Steps-per-mm
       RTS_SndDataL(planner.axis_steps_per_mm[B_AXIS]*100, AdvancedMenu+2); // Y Steps-per-mm
       RTS_SndDataL(planner.axis_steps_per_mm[C_AXIS]*100, AdvancedMenu+4); // Z Steps-per-mm
       RTS_SndDataL(planner.axis_steps_per_mm[E_AXIS]*100, AdvancedMenu+6); // E Steps-per-mm
       //
       RTS_SndDataL(planner.max_feedrate_mm_s[A_AXIS]*100, AdvancedMenu+8);  // X Max Feedrate
       RTS_SndDataL(planner.max_feedrate_mm_s[B_AXIS]*100, AdvancedMenu+16); // Y Max Feedrate
       RTS_SndDataL(planner.max_feedrate_mm_s[C_AXIS]*100, AdvancedMenu+18); // Z Max Feedrate
       RTS_SndDataL(planner.max_feedrate_mm_s[E_AXIS]*100, AdvancedMenu+20); // E Max Feedrate
       //
       RTS_SndDataL(planner.max_acceleration_mm_per_s2[A_AXIS], AdvancedMenu+22); // X Max Acceleration
       RTS_SndDataL(planner.max_acceleration_mm_per_s2[B_AXIS], AdvancedMenu+24); // Y Max Acceleration
       RTS_SndDataL(planner.max_acceleration_mm_per_s2[C_AXIS], AdvancedMenu+32); // Z Max Acceleration
       RTS_SndDataL(planner.max_acceleration_mm_per_s2[E_AXIS], AdvancedMenu+34); // E Max Acceleration
       //
       RTS_SndDataL(planner.max_jerk[A_AXIS]*100, AdvancedMenu+36); // X Jerk
       RTS_SndDataL(planner.max_jerk[B_AXIS]*100, AdvancedMenu+38); // Y Jerk
       RTS_SndDataL(planner.max_jerk[C_AXIS]*100, AdvancedMenu+40); // Z Jerk
       RTS_SndDataL(planner.max_jerk[E_AXIS]*100, AdvancedMenu+48); // E Jerk
       //
       RTS_SndDataL(planner.acceleration*100, AdvancedMenu+50); // Acceleration
       RTS_SndDataL(planner.retract_acceleration*100, AdvancedMenu+52); // Retract Acceleration
       RTS_SndDataL(planner.min_feedrate_mm_s*100, AdvancedMenu+54); // Travel Acceleration
       //
       delay(5);
       RTS_SndData(ExchangePageBase + 89, ExchangepageAddr); //exchange to 89 page
     }
     else if(recdat.addr == AdvancedMenu) // X Steps-per-mm
     {
        planner.axis_steps_per_mm[A_AXIS] = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+2) // Y Steps-per-mm
     {
        planner.axis_steps_per_mm[B_AXIS] = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+4) // Z Steps-per-mm
     {
        planner.axis_steps_per_mm[C_AXIS] = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+6) // E Steps-per-mm
     {
        planner.axis_steps_per_mm[E_AXIS] = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+8) // X Max Feedrate
     {
        planner.max_feedrate_mm_s[A_AXIS] = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+16) // Y Max Feedrate
     {
        planner.max_feedrate_mm_s[B_AXIS] = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+18) // Z Max Feedrate
     {
        planner.max_feedrate_mm_s[C_AXIS] = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+20) // E Max Feedrate
     {
        planner.max_feedrate_mm_s[E_AXIS] = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+22) // X Max Acceleration
     {
        planner.max_acceleration_mm_per_s2[A_AXIS] = recdat.data[0];
  	 }
     else if(recdat.addr == AdvancedMenu+24) // Y Max Acceleration
     {
        planner.max_acceleration_mm_per_s2[B_AXIS] = recdat.data[0];
  	 }
     else if(recdat.addr == AdvancedMenu+32) // Z Max Acceleration
     {
        planner.max_acceleration_mm_per_s2[C_AXIS] = recdat.data[0];
  	 }
     else if(recdat.addr == AdvancedMenu+34) // E Max Acceleration
     {
        planner.max_acceleration_mm_per_s2[E_AXIS] = recdat.data[0];
  	 }
     else if(recdat.addr == AdvancedMenu+36) // X Jerk
     {
        planner.max_jerk[A_AXIS] = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+38) // Y Jerk
     {
        planner.max_jerk[B_AXIS] = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+40) // Z Jerk
     {
        planner.max_jerk[C_AXIS] = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+48) // E Jerk
     {
        planner.max_jerk[E_AXIS] = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+50) // Acceleration
     {
        planner.acceleration = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+52) // Retract Acceleration
     {
        planner.retract_acceleration = ((float)recdat.data[0]/100);
  	 }
     else if(recdat.addr == AdvancedMenu+54) // Travel Acceleration
     {
        planner.min_feedrate_mm_s = ((float)recdat.data[0]/100);
  	 }
     // END AdvancedMenu ##

	// EEPROM UTILS
	 if (recdat.addr == AdvancedMenu + 82 && recdat.data[0] == 0xF1) // LOAD 1952
		 {
			 settings.load();
		 }
		 else if (recdat.addr == AdvancedMenu + 84 && recdat.data[0] == 0xF1) // SAVE 1954
		 {
			 settings.save();
		 }
		 else if (recdat.addr == AdvancedMenu + 86 && recdat.data[0] == 0xF1) // RE-INIT EEPROM 1956 
		 // 1958 / 195A / 195C
		 {
			 settings.reset();
		 }

	 if(recdat.addr == AdvancedMenu+94) // BL-Touch UTILS
      {
        if(recdat.data[0] == 1) // BLTOUCH_RESET
         {
             enqueue_and_echo_commands_P(PSTR("M280 P" STRINGIFY(Z_PROBE_SERVO_NR) " S" STRINGIFY(BLTOUCH_RESET)));
         }
        if(recdat.data[0] == 2) // BLTOUCH_SELFTEST
         {
             enqueue_and_echo_commands_P(PSTR("M280 P" STRINGIFY(Z_PROBE_SERVO_NR) " S" STRINGIFY(BLTOUCH_SELFTEST)));
         }
         else if(recdat.data[0] == 3) // BLTOUCH_DEPLOY (DOWN)
         {
             enqueue_and_echo_commands_P(PSTR("M280 P" STRINGIFY(Z_PROBE_SERVO_NR) " S" STRINGIFY(BLTOUCH_DEPLOY)));
         }
         else if(recdat.data[0] == 4) // BLTOUCH_STOW (UP)
         {
             enqueue_and_echo_commands_P(PSTR("M280 P" STRINGIFY(Z_PROBE_SERVO_NR) " S" STRINGIFY(BLTOUCH_STOW)));
         }
     }
